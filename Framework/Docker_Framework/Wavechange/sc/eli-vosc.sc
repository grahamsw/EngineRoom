...
s.waitForBoot({
    ...
    SynthDef("help-VOsc",{ arg out = 0, bufoffset = 0, freq=120, bufindex=0.5;
    Out.ar(out,
        VOsc.ar(bufindex, freq, phase, amp)
    )
}).add;
	...
	/*
	Create two VOsc Synths, reading their wavetable
	...
	~synths = [~i0Buf, ~i1Buf].collect{
		arg buf, i;
		{
			...
			*/
			...
			var sig;
            var phase = {Rand(0,2pi)}!8;
            var a_freq = freq * LFNoise1.kr({Rand(0.08,0.15)}!8).bipolar(detune).midiratio
			sig = Osc.ar(
				buf.bufnum,
